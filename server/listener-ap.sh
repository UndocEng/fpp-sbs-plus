...
# Uses <Location /> with RewriteEngine On — this is processed AFTER <Directory>
# blocks, so it gets the final say even though FPP's <Directory /opt/fpp/www/>
# has AllowOverride None which prevents .htaccess rewrites.
...
if [[ ${#LISTENER_INTERFACES[@]} -gt 0 ]]; then
...

    {
        ...
        echo "# Uses <Location> context — processed AFTER <Directory>, survives FPP VirtualHost regen"
        echo ""
        echo "<Location />"
    ...
    # Redirect everything else from ${L_SUBNET}.x to listen page
